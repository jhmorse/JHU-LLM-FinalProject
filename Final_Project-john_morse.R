...
fname <- paste0("documents_signed_in_2025_signed_by_",
                ...
                "and_of_presidential_document_type_executive_order.csv")
...
## the clean_text column is now a reference library

#################################################################
## Prep section: Loading an LLM for embedding, embedding the RAG
## corpus, and loading an LLM for text generation.

# Read in chunked data for RAG corpus
chunked_data <- read_csv("chunked_eo_corpus_john_morse.csv")
...
# Copy the data frame to preserve the original data
embed_corpus <- chunked_data
...
###################################################################
## Function to return LLM results based on question submitted.

get_answer <- function(aquery, 
                       ragdf = embed_corpus, 
                       embed_model = model, 
                       top_k = 3,
                       LLMgenerator = generator) {
  
  # Get the top 5 results from the embedded corpus
...
get_doc_summaries <- function(aquery,
  ...
  for (i in seq_len(nrow(relevant_docs))) {
    ...
    request <- paste0("Provide a summary of the document above in three bullet points. ",
    ...
    summary_prompt <- sprintf(
      #"<DOCUMENT>\n%s\n</DOCUMENT>\n\n<REQUEST>\nProvide a summary of the document above in three bullet points. You are analyzing executive orders signed by President Donald Trump in 2025. Do not attribute them to any other president.\n</REQUEST><SUMMARY>",
      "<DOCUMENT>\n%s\n</DOCUMENT>\n\n<REQUEST>\n%s\n</REQUEST><SUMMARY>",
      doc_text, request
    )
